...
- [ ] `backend/.env` filled in (at least `MONGODB_URI` and `GEMINI_API_KEY`)
...
HomeScreen API running on http://localhost:5000
```
...
**"CORS error":**
- Make sure `VITE_API_URL` in `frontend/.env` is `http://localhost:5000`
